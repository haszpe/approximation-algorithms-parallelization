...
def NW_matrix(matrix, num_current, num_next_right, num_next_down, match=2, mismatch=-1, gap=-2):
    ...
    for y_item in range(1, n+1):
        for x_item in range(1, m+1):
            if x[x_item-1] == y[y_item-1]:
                diag = d[y_item-1][x_item-1] + match
            else:
                diag = d[y_item-1][x_item-1] + mismatch

            d[y_item, x_item] = np.max([diag,
...
def dicts_creation(A, B, processes, gap):
    ...
    for y in ys:            #po kolumnach
        for x in xs:        #po wierszach
            ...
            if num in pierwszy_wiersz:
                if num == 1:
                    granica_up = [temp_m + (_ * gap) for _ in range(0, len(x)+1)]
                    granica_up_save = granica_up
                else:
                    temp_m = granica_up_save[-1]
                    granica_up = [temp_m + (_ * gap) for _ in range(0, len(x)+1)]
                    granica_up_save = granica_up
            else:
                granica_up = None
            ...
            if num in pierwsza_kolumna:
                if num == 1:
                    granica_left = [temp_n + (_ * gap) for _ in range(0, len(y)+1)]
                    granica_left_save = granica_left
                else:
                    temp_n = granica_left_save[-1]
                    granica_left = [temp_n + (_ * gap) for _ in range(0, len(y)+1)]
                    granica_left_save = granica_left
            else:
                granica_left = None
...
def NW_matrix_worker(matrix, num_current, num_next_right, num_next_down):
    return NW_matrix(matrix, num_current, num_next_right, num_next_down)


def needleman_wunsch_traceback(full_matrix, x, y, gap):
    dopasowana_1 = ""
    dopasowana_2 = ""
    i = len(x)
    j = len(y)
...
if __name__ == "__main__":
    ...
    manager = Manager()
    matrix = manager.dict(dicts_creation(A, B, processes, gap))
    ...
    for akt in akty:
        ...
        #Stworzenie rownoległych procesów dla danego aktu
            ...
            pool = Pool(processes)
            ld_results = pool.starmap(NW_matrix_worker, tasks)
            pool.close()
            pool.join()
    ...
    #Obliczenie mmaksymalnej wartości
    ind = np.unravel_index(np.argmax(filtered), filtered.shape)
    max_i = ind[0]
    max_j = ind[1]
    aligned_x, aligned_y = needleman_wunsch_traceback(filtered, A, B, gap)
    print(f"Parallelized Needleman-Wunsch algorithm took: {(datetime.now() - start).seconds} s")
